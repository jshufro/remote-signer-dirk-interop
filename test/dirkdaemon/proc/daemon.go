// Adapted from https://github.com/attestantio/dirk/blob/567e879f4028e3057c28a3833061bcd413f4bdb9/testing/daemon/daemon.go
...
// - distributed wallet 'Wallet 3' with the test distributed wallet
...
func New(ctx context.Context, path string, id uint64, port uint32, peersMap map[uint64]string) (*logger.LogCapture, string, error) {
	...
	wallet3, err := distributed.CreateWallet(ctx, "Wallet 3", store, encryptor)
	if err != nil {
	...
	if daccImporter, ok := wallet3.(e2wtypes.WalletDistributedAccountImporter); ok {
		...
		for account := range testWallet.Accounts(ctx) {
			locker := account.(e2wtypes.AccountLocker)
			err := locker.Unlock(ctx, []byte{})
			if err != nil {
				return nil, "", errors.Wrap(err, "failed to unlock account")
			}
			distributedAccount := account.(e2wtypes.DistributedAccount)
			privateKeyProvider := account.(e2wtypes.AccountPrivateKeyProvider)
			vvecProvider := account.(e2wtypes.AccountVerificationVectorProvider)
			privateKey, err := privateKeyProvider.PrivateKey(ctx)
			if err != nil {
				return nil, "", errors.Wrap(err, "failed to get private key")
			}
			vvec := vvecProvider.VerificationVector()
			vvecBytes := make([][]byte, len(vvec))
			for i, v := range vvec {
				vvecBytes[i] = v.Marshal()
			}
			_, err = daccImporter.ImportDistributedAccount(ctx, account.Name(), privateKey.Marshal(), distributedAccount.SigningThreshold(), vvecBytes, distributedAccount.Participants(), []byte("pass"))
			if err != nil {
				return nil, "", errors.Wrap(err, "failed to import distributed account")
			}
		}
	} else {
		return nil, "", errors.New("wallet 3 does not implement WalletDistributedAccountImporter")
	}
	...
	rules, err := standardrules.New(ctx,
		...
		standardrules.WithAdminIPs([]string{"127.0.0.1"}),
	)
	if err != nil {
